...
> Hướng dẫn từ A → Z cho dự án LUUHIEN-SPRING-REST-WITH-AI.
> Áp dụng cho Claude Code, Cursor, hoặc bất kỳ AI coding tool nào.

## Update 2026-04-07 (Copilot)

- GitHub Copilot đang dùng bộ customizations trong `.github/`.
- Slash commands cho Copilot nằm trong `.github/prompts/*.prompt.md`.
- Workflow dài được tách vào `.github/skills/*/SKILL.md`.
- `.claude/commands/` vẫn được giữ làm legacy reference, không xóa.
...
│
├── .github/
│   ├── instructions/                  # Always-on guidance cho Copilot
│   ├── prompts/                       # Slash commands của Copilot
│   └── skills/                        # Workflow tái sử dụng (Copilot)
│
├── docs/
